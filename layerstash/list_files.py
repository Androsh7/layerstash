...
# Project libraries
from layerstash.docker_api import get_tag_list, get_manifest
from layerstash.utils import humanize_bytes
from layerstash.constants import config
...
def list_files(exact_size: bool = False):
    ...
    for base_tag, chunks in base_tag_dict.items():
        if exact_size:
            ...
            size = 5 * 1024 * 1024 * 1024 * (chunks -1 ) + last_manifest.layers[0].size # 5GB * number of full chunks + last chunk size
